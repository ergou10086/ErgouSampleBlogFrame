# ErgouSampleBlogFrame

一个极为简洁的博客系统框架，使用 Node.js + Express + EJS + Supabase 构建，支持 Markdown 格式文章编写，适合学习JS,EJS和小型项目内嵌使用。

## ✨ 功能特性

- ✅ **用户认证系统** - 基于 Supabase Auth 的用户注册、登录功能
- ✅ **文章管理** - 创建、编辑、删除博客文章
- ✅ **Markdown 支持** - 使用 Markdown 格式编写文章，自动渲染为 HTML
- ✅ **分页展示** - 首页文章列表支持分页浏览
- ✅ **响应式设计** - 使用 Bootstrap 5，适配各种设备
- ✅ **权限控制** - 用户只能编辑和删除自己的文章
- ✅ **浏览量统计** - 自动统计文章浏览量
- ✅ **草稿功能** - 支持保存草稿和发布文章

## 🛠️ 技术栈

- **后端框架**: Node.js + Express
- **模板引擎**: EJS (服务端渲染)
- **数据库**: Supabase (PostgreSQL)
- **认证服务**: Supabase Auth
- **Markdown 渲染**: Marked
- **前端框架**: Bootstrap 5
- **图标库**: Bootstrap Icons

## 📁 项目结构

```
ErgouSampleBlogFrame/
├── src/
│   ├── config/
│   │   └── supabase.js          # Supabase 配置
│   ├── controllers/
│   │   ├── authController.js    # 认证控制器
│   │   ├── postController.js    # 文章控制器
│   │   └── userController.js    # 用户控制器
│   ├── middlewares/
│   │   ├── auth.js              # 认证中间件
│   │   └── errorHandler.js      # 错误处理中间件
│   ├── routes/
│   │   ├── auth.js              # 认证路由
│   │   ├── posts.js             # 文章路由
│   │   └── index.js             # 首页路由
│   └── views/
│       ├── layouts/
│       │   └── main.ejs         # 主布局模板
│       ├── partials/
│       │   ├── header.ejs       # 头部组件
│       │   └── footer.ejs       # 底部组件
│       ├── index.ejs            # 首页
│       ├── login.ejs            # 登录页
│       ├── register.ejs         # 注册页
│       ├── post-detail.ejs      # 文章详情页
│       ├── post-edit.ejs        # 编辑文章页
│       ├── post-create.ejs      # 创建文章页
│       └── error.ejs            # 错误页
├── public/                      # 静态资源目录
│   ├── css/
│   ├── js/
│   └── images/
├── sql/                         # 数据库脚本
│   ├── supabase.sql            # 数据库表结构
│   ├── complete_setup.sql      # 完整设置脚本（推荐）
│   ├── create_user_profile_trigger.sql  # 用户资料触发器
│   └── fix_user_profiles_insert.sql     # 修复脚本
├── .env                        # 环境变量（需要创建）
├── .env.example                # 环境变量示例
├── app.js                      # 应用入口文件
├── package.json
└── README.md
```

## 🚀 快速开始

### 1. 克隆项目

```bash
git clone <your-repo-url>
cd ErgouSampleBlogFrame
```

### 2. 安装依赖

```bash
npm install
```

### 3. 配置 Supabase

#### 3.1 创建 Supabase 项目

1. 访问 [Supabase](https://supabase.com) 并创建账号
2. 创建一个新项目
3. 等待项目初始化完成

#### 3.2 设置数据库

在 Supabase Dashboard 的 SQL Editor 中执行以下 SQL 脚本：

**推荐方式**：执行 `sql/complete_setup.sql`（包含所有必要的表、策略和触发器）

或者分步执行：
1. 先执行 `sql/supabase.sql` 创建表结构
2. 再执行 `sql/create_user_profile_trigger.sql` 创建触发器

#### 3.3 获取 API 密钥

在 Supabase Dashboard 的 Project Settings > API 中获取：
- **Project URL**
- **anon/public key**

#### 3.4 配置认证

在 Supabase Dashboard 的 Authentication > Settings 中：
- 确保 Email 认证已启用
- 根据需要配置邮箱确认（建议开发环境关闭，生产环境开启）

### 4. 配置环境变量

复制 `.env.example` 为 `.env`：

```bash
cp .env.example .env
```

编辑 `.env` 文件，填入你的配置：

```env
# Supabase 配置
SUPABASE_URL=your_supabase_project_url
SUPABASE_ANON_KEY=your_supabase_anon_key

# Session 密钥（生产环境请使用强随机字符串）
SESSION_SECRET=your-session-secret-key-change-this

# 服务器端口
PORT=3000

# 环境
NODE_ENV=development
```

### 5. 运行项目

**开发模式**（使用 nodemon，自动重启）：

```bash
npm run dev
```

**生产模式**：

```bash
npm start
```

### 6. 访问应用

打开浏览器访问：`http://localhost:3000`

## 📖 使用说明

### 注册账号

1. 访问 `/auth/register` 或点击导航栏的"注册"
2. 填写用户名、邮箱和密码（至少6位）
3. 点击注册按钮
4. 如果启用了邮箱确认，请检查邮箱并点击确认链接

### 登录

1. 访问 `/auth/login` 或点击导航栏的"登录"
2. 输入邮箱和密码
3. 点击登录

### 发布文章

1. 登录后，点击导航栏的"写文章"按钮
2. 填写文章标题和内容（支持 Markdown 格式）
3. 选择状态：发布 或 草稿
4. 点击"发布文章"按钮

### Markdown 语法示例

```markdown
# 一级标题
## 二级标题
### 三级标题

**粗体文本**
*斜体文本*

- 无序列表项 1
- 无序列表项 2

1. 有序列表项 1
2. 有序列表项 2

`行内代码`

```代码块
function hello() {
  console.log('Hello World!');
}
```

> 引用文本

[链接文本](https://example.com)
```

### 编辑文章

1. 在文章详情页，点击"编辑"按钮（仅作者可见）
2. 修改文章内容
3. 点击"保存更改"

### 删除文章

1. 在文章详情页，点击"删除"按钮（仅作者可见）
2. 确认删除操作

## 🗄️ 数据库说明

### 数据表

- **user_profiles**: 用户扩展信息表
- **posts**: 文章表
- **categories**: 分类表（预留，当前未使用）

### Row Level Security (RLS)

项目启用了 Supabase 的 RLS 功能，确保数据安全：
- 用户可以查看所有已发布的文章
- 用户只能编辑和删除自己的文章
- 用户只能创建和更新自己的用户资料

## ⚙️ 配置说明

### 请求体大小限制

默认支持最大 50MB 的文章内容。如需调整，修改 `app.js` 中的：

```javascript
app.use(express.json({ limit: '50mb' }));
app.use(express.urlencoded({ extended: true, limit: '50mb' }));
```

### 分页设置

默认每页显示 10 篇文章。如需调整，修改 `src/controllers/postController.js` 中的：

```javascript
const limit = 10; // 修改为你想要的数量
```

## 🐛 常见问题

### 1. 注册时提示"Database error saving new user"

**解决方案**：
- 确保已执行 `sql/complete_setup.sql` 或 `sql/create_user_profile_trigger.sql`
- 检查 Supabase 项目设置中是否启用了 Email 认证

### 2. 文章无法显示

**解决方案**：
- 检查文章状态是否为 "published"
- 查看服务器控制台是否有错误信息
- 确认数据库连接正常

### 3. 页脚位置不正确

**解决方案**：
- 确保浏览器支持 Flexbox（现代浏览器都支持）
- 清除浏览器缓存后刷新页面

## 🔒 安全建议

在生产环境部署前，请确保：

1. **修改 Session Secret**：使用强随机字符串
   ```bash
   # 生成随机字符串
   node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
   ```

2. **启用 HTTPS**：配置 SSL 证书

3. **配置环境变量**：不要将 `.env` 文件提交到版本控制

4. **设置 Supabase RLS 策略**：根据实际需求调整安全策略

5. **启用邮箱确认**：生产环境建议启用邮箱验证

## 🚀 部署指南

### 部署到 Netlify

Netlify 支持通过 Serverless Functions 部署 Express 应用。项目已配置好相关文件。

#### 前置准备

1. 确保项目已推送到 Git 仓库（GitHub、GitLab 或 Bitbucket）
2. 在 Netlify 上创建账号并登录

#### 部署步骤

**方式一：通过 Netlify Dashboard 部署（推荐）**

1. 访问 [Netlify Dashboard](https://app.netlify.com/)
2. 点击 "Add new site" > "Import an existing project"
3. 选择你的 Git 提供商并授权
4. 选择项目仓库
5. 配置构建设置：
   - **Build command**: `npm install`
   - **Publish directory**: `.netlify`（或留空）
   - **Functions directory**: `netlify/functions`
6. 点击 "Show advanced" 添加环境变量：
   ```
   SUPABASE_URL=your_supabase_project_url
   SUPABASE_ANON_KEY=your_supabase_anon_key
   SESSION_SECRET=your-session-secret-key
   NODE_ENV=production
   ```
7. 点击 "Deploy site"

**方式二：使用 Netlify CLI**

1. 安装 Netlify CLI：
   ```bash
   npm install -g netlify-cli
   ```

2. 登录 Netlify：
   ```bash
   netlify login
   ```

3. 在项目根目录初始化：
   ```bash
   netlify init
   ```

4. 设置环境变量：
   ```bash
   netlify env:set SUPABASE_URL your_supabase_project_url
   netlify env:set SUPABASE_ANON_KEY your_supabase_anon_key
   netlify env:set SESSION_SECRET your-session-secret-key
   netlify env:set NODE_ENV production
   ```

5. 部署：
   ```bash
   netlify deploy --prod
   ```

#### 注意事项

- Netlify Functions 有执行时间限制（免费版 10 秒，Pro 版 26 秒）
- Session 存储可能需要使用外部存储（如 Redis）或数据库
- 建议使用 Netlify 的环境变量管理功能，不要将敏感信息提交到代码仓库
- **如果遇到 pnpm-lock.yaml 错误**：
  - 项目已配置为使用 npm，如果仓库中有 `pnpm-lock.yaml` 文件，可以删除它
  - 或者确保 `pnpm-lock.yaml` 与 `package.json` 同步（在本地运行 `pnpm install` 后提交）
  - `netlify.toml` 中已设置 `NETLIFY_USE_PNPM = "false"` 强制使用 npm

### 其他部署平台推荐

由于 Express SSR 应用的特性，以下平台可能更适合：

#### 1. Railway（推荐）

Railway 对 Node.js 应用支持很好，部署简单：

1. 访问 [Railway](https://railway.app/)
2. 使用 GitHub 账号登录
3. 点击 "New Project" > "Deploy from GitHub repo"
4. 选择项目仓库
5. 添加环境变量
6. 自动部署完成

#### 2. Render

1. 访问 [Render](https://render.com/)
2. 创建账号并连接 GitHub
3. 点击 "New" > "Web Service"
4. 选择项目仓库
5. 配置：
   - **Build Command**: `npm install`
   - **Start Command**: `npm start`
   - 添加环境变量
6. 点击 "Create Web Service"

#### 3. Vercel

Vercel 对 Node.js 支持很好，但需要稍微调整配置：

1. 访问 [Vercel](https://vercel.com/)
2. 使用 GitHub 账号登录
3. 导入项目
4. 配置环境变量
5. 部署

#### 4. Heroku

1. 安装 Heroku CLI
2. 登录：`heroku login`
3. 创建应用：`heroku create your-app-name`
4. 设置环境变量：`heroku config:set KEY=value`
5. 部署：`git push heroku main`

## 📝 开发说明

这是一个学习项目，适合：
- 学习 Node.js 和 Express 框架
- 理解服务端渲染（SSR）
- 学习 Supabase 的使用
- 作为小型博客项目的基础框架

## 📄 License

ISC

## 🙏 致谢

- [Express](https://expressjs.com/)
- [Supabase](https://supabase.com/)
- [Bootstrap](https://getbootstrap.com/)
- [Marked](https://marked.js.org/)

## 📧 联系方式

如有问题或建议，欢迎提交 Issue 或 Pull Request。

---

**Happy Blogging! 🎉**

